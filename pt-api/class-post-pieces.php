...
class SELLMYHOME_pt_pcs{
	...
	//General
		...
		public static function pc_content( $quer = null ){
			...
			$out = "";
	   		$out .= "<div class=' SELLMYHOME-content '>";
		...
		public static function pc_excerpt( $quer = null ){
			...
			$out = "";
	   		$out .= "<div class=' SELLMYHOME-excerpt '>";
	   			$exc =  get_the_excerpt();
	   			$out .= "<p>";
		   			$out .= substr( $exc, 0, -11 );
		...
		public static function pc_fi( $quer = null ){
	    	...
	    	if ( has_post_thumbnail( $post->ID ) ){
		    	$out .= "<div class='" . "SELLMYHOME-image" . "'>";
		...
		public static function pc_meta( $quer = null ){
			...
			$meta = SELLMYHOME_func::print_meta( $post->ID );

			$out = "";

			//Check if there is actually any content, if not don't output.
			if ( strlen( $meta ) > 10 ){
		   		$out .= "<div class='" . "SELLMYHOME-meta" . "'>";
				$out .= $meta;
			    $out .= "</div>";
			}
		...
		public static function pc_media( $quer = null ){
			...
			$media = SELLMYHOME_func::print_media( $post->ID );

			$out = "";

			if ( strlen( $media ) > 29 ){
		   		$out .= "<div class='" . "SELLMYHOME-media" . "'>";
				$out .= $media;
			    $out .= "</div>";
			}
		...
		public static function pc_cats( $quer = null ){
			...
			if ($has_terms){
		    	$out .= "<div class='" . "SELLMYHOME-categories" . "'>";
			    	foreach($pc_values as $tax){
			    		$out .= SELLMYHOME_func::get_cats( $tax , $post->ID , $post->post_type );
			    	}
		    	$out .= "</div>";
	    	}	
