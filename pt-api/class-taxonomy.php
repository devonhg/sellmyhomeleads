...
class SELLMYHOME_pt_tax{
	...
	public function __construct($name, $name_s, $pt_slug, $hierarchical = true, $args = null){
        $this->name = $name;
        $this->name_s = $name_s;
        $this->pt_slug = $pt_slug;
        $this->tax_slug = str_replace( " ", "_", "tax_" . trim(strtolower($name_s)) . "_" . substr($pt_slug, 3) );
        $this->args = $args; 
        $this->hierarchical = $hierarchical;

        SELLMYHOME_pt_tax::$instances[] = $this; 

        //Get Parent
        foreach( SELLMYHOME_post_type::$instances as $inst ){
